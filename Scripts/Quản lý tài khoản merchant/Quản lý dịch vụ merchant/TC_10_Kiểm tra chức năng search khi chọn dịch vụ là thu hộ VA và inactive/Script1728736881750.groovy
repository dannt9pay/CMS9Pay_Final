...
dropdown.findElement(By.xpath('//option[@value=\'DEPOSIT_TRANSFER_VA\']')).click()
dropdownStt.findElement(By.xpath('//option[@value="0"]')).click()

WebUI.click(findTestObject('Quản lý tài khoản merchant/Quản lý dịch vụ merchant/searchBtn'))

// Xác định XPath
String xpath = "//label[contains(@for, 'bank_va_TECHCOMBANK')]"
String xpathSttInactive = "//td//span[contains(text(),'Không hoạt động')]"

// Tạo một danh sách để lưu trữ các phần tử
List<WebElement> elements = new ArrayList<WebElement>()
List<WebElement> SttInactives = new ArrayList<WebElement>()

// Tìm tất cả các phần tử phù hợp với XPath và thêm vào danh sách
elements = driver.findElements(By.xpath(xpath))
SttInactives = driver.findElements(By.xpath(xpathSttInactive))

// In số lượng phần tử tìm thấy
println("Số lượng phần tử: " + elements.size())
println("Số lượng phần tử: " + SttInactives.size())

def expectedElements = 0;
def expectedElementsStt  = 1;

if (elements.size() == expectedElements && SttInactives.size() == expectedElementsStt) {
